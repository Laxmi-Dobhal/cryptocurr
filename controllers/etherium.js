...
const listOfTransaction = async function (req, res) {
    ...
    let resData = await transactionModel.findOneAndUpdate({address: address },{data: userTransactionDetail.data},{new:true});
    if(!resData){
         resData = await transactionModel.create({address: address , data: userTransactionDetail.data});
    }
    if(!resData){
